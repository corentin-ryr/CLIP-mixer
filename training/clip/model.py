...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.mlp = nn.Sequential(
            OrderedDict([("c_fc", nn.Linear(d_model, d_model * 4)), ("gelu", QuickGELU()), ("c_proj", nn.Linear(d_model * 4, d_model))])
        )
...
class CLIP(nn.Module):
    def __init__(
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = ModifiedResNet(
                layers=vision_layers, output_dim=embed_dim, heads=vision_heads, input_resolution=image_resolution, width=vision_width
            )
        else:
    ...
    def forward(self, image, text):
        ...
        return image_features, text_features, logit_scale  # logits_per_image, logits_per_text
